...
public class BuscarNecessidadesComoCasaServiceTest {

    @InjectMocks
    private BuscarNecessidadesComoCasaService service;
    ...
    public void ok() {
        ...
        service.buscar(pagina, tamanho, ordenacao, direcao, categorias, subcategorias, statuses, mesesCorte, texto, idUsuarioLogado);

        verify(buscarPedidosComFiltrosService).buscar(eq(pagina), eq(tamanho), eq(ordenacao), eq(direcao), eq(categorias),
                eq(subcategorias), eq(statuses), eq(mesesCorte), eq(texto), eq(NECESSIDADE), eq(idUsuarioLogado));
    }
}
