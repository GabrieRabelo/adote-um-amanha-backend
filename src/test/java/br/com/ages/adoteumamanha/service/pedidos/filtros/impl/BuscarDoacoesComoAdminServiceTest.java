...
public class BuscarDoacoesComoAdminServiceTest {

    @InjectMocks
    private BuscarDoacoesComoAdminService service;
    ...
    public void ok() {
        ...
        verify(buscarPedidosComFiltrosService).buscar(eq(pagina), eq(tamanho), eq(ordenacao), eq(direcao), eq(categorias),
                eq(subcategorias), eq(statuses), eq(mesesCorte), eq(texto), eq(DOACAO), eq(null));
    }
}
