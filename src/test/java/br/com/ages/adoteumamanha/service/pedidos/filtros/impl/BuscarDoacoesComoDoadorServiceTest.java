...
public class BuscarDoacoesComoDoadorServiceTest {

    @InjectMocks
    private BuscarDoacoesComoDoadorService service;
    ...
    public void ok() {
        ...
        verify(buscarPedidosComFiltrosService).buscar(eq(pagina), eq(tamanho), eq(ordenacao), eq(direcao), eq(categorias),
                eq(subcategorias), eq(statuses), eq(mesesCorte), eq(texto), eq(DOACAO), eq(idUsuarioLogado));
    }
}
